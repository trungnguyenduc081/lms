<div class="space-y-6">
    ...
    <div>
        ...
        <x-text-input id="password" name="password" type="password" class="mt-1 block w-full" autocomplete="password" placeholder="Password"/>
        <x-input-error class="mt-2" :messages="$errors->get('password')"/>
    </div>
    <div>
        <x-input-label for="permissions" :value="__('Permissions')"/>
        <x-select id="permissions" multiple="multiple" name="permissions[]" :options="config('permission.basic')" :value="old('permissions', $user?->permissions)"/>
